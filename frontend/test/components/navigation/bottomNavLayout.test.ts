...
describe('getExpandedFlexGrow', () => {
  it('makes one of three items exactly 20% wider on hover', () => {
  ...
  it('makes one of three items exactly 30% wider when pressed', () => {
    const targetWeight = getExpandedFlexGrow(3, 1.3)
    const targetShare = targetWeight / (targetWeight + 2)

    expect(targetShare).toBeCloseTo(1.3 / 3)
  })
