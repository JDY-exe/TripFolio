/**
 * Material 3 expressive loading-indicator shapes.
 *
 * Adapted for TripFolio from Aler1x/m3-loading-indicator. Shape path data is
 ...
 *
 * @param path - SVG path containing move, line, cubic, and close commands.
...
/**
 * Samples a cubic Bézier after its start point to retain its curved outline.
 *
 * @param start - Curve start point.
 * @param controlOne - First control point.
 * @param controlTwo - Second control point.
 * @param end - Curve end point.
 * @param steps - Number of evenly timed samples.
 * @returns Sampled points along the curve.
 */
function sampleCubic(
  start: LoadingIndicatorPoint,
  controlOne: LoadingIndicatorPoint,
  controlTwo: LoadingIndicatorPoint,
...
/**
 * Samples supported SVG commands into a continuous point outline.
 *
 * @param path - Material shape SVG path.
...
/**
 * Centers and scales source points into Material's normalized square.
 *
 * @param points - Points in source SVG coordinates.
 * @param viewBox - Width and height of the square source view box.
 * @returns Centered points bounded by -1 and 1.
 */
...
/**
 * Redistributes an outline into equal arc-length samples for stable morphing.
 *
 * @param points - Ordered points around a closed outline.
 * @param count - Desired output sample count.
 * @returns Equally spaced points around the outline.
 */
...
/**
 * Generates the final Material oval with the same point count as SVG shapes.
 *
 * @param count - Number of samples around the oval.
 * @returns Normalized horizontal oval points.
 */
 ...
 * Spring overshoot naturally selects the following outline before settling.
 *
 * @param morph - Continuous position through the seven-shape sequence.
 * @returns A normalized point array ready for canvas drawing.
 */
